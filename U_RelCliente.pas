...
type
  TF_RelCliente = class(TForm)
    FDQ_Relatorio: TFDQuery;
    DataSource1: TDataSource;
    FDQ_Relatoriocodigo: TFDAutoIncField;
    FDQ_Relatorionome: TStringField;
    FDQ_Relatorioendereco: TStringField;
    FDQ_Relatoriotelefone: TStringField;
    ...
    RelCliente: TfrxDBDataset;
    frx_RelCliente: TfrxReport;
    ...
    FDQ_Relatoriocidade: TStringField;
    FDQ_Relatorioemail: TStringField;
...
procedure TF_RelCliente.SomenteNumeros(Sender: TObject; var Key: Char);
begin
  if not (Key in['0'..'9', Chr(8)]) then
    Key:= #0;
end;

procedure TF_RelCliente.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  F_RelCliente := nil;
end;

procedure TF_RelCliente.SB_imprimirClick(Sender: TObject);
var vOrdem : String;
begin
  ...
  FDQ_Relatorio.SQL.Add('select * from tb_cliente where codigo >=' + Edit1.Text);
  ...
  case CB_Ordenacao.ItemIndex of
    ...
    1: begin
      FDQ_Relatorio.SQL.Add(' order by nome');
      vOrdem := ' ordenado por nome';
    end;
  end;
  FDQ_Relatorio.Active := True;
  frx_RelCliente.ShowReport();
...
procedure TF_RelCliente.SB_voltarClick(Sender: TObject);
begin
  Close;
end;
